 ...
 * @returns {cheerio}
 */
function page_parser$1(responseText) {
 ...
 * @returns {Response}
 */
function makeJsonRawResponse(body, initOverride) {
 ...
 * @returns {Response}
 */
function makeJsonResponse(body_update, env) {
  const body = {
...
async function fetchWithTimeout$4(url, opts = {}, timeout = DEFAULT_TIMEOUT$4) {
...
const safe$3 = (v, fallback = '') => (v === undefined || v === null ? fallback : v);

async function gen_douban(sid, env) {
  ...
  try {
    ...
    let resp = await fetchWithTimeout$4(baseLink, { headers }, DEFAULT_TIMEOUT$4);
    if (!resp || (resp.status === 204 || resp.status === 403 || resp.status === 521 || resp.status === 521)) {
      // 尝试移动端页面回退
      try {
        const mresp = await fetchWithTimeout$4(mobileLink, { headers }, DEFAULT_TIMEOUT$4);
    ...
    data.chinese_title = safe$3(title, '');
    data.foreign_title = safe$3($('span[property="v:itemreviewed"]').text().replace(data.chinese_title, "").trim(), '');
    ...
    data.year = safe$3($("#content > h1 > span.year").text().match(/\d{4}/)?.[0] || "", "");
    ...
    const douban_avg = safe$3(rating_info.ratingValue || page_rating_average || '0', '0');
    const douban_votes = safe$3(rating_info.ratingCount || page_votes || '0', '0');
    ...
    if (imdb_anchor.length > 0) {
      ...
      if (imdb_id) {
        ...
        imdb_api_req = fetchWithTimeout$4(imdb_jsonp_url, { headers }, 8000).catch(() => null);
      }
    }

    // 异步请求 awards 页面与 IMDb（并行）
    const awardsReq = fetchWithTimeout$4(`${baseLink}awards`, { headers }, 8000).catch(() => null);
    let awards = '';
    try {
      const awardsResp = await awardsReq;
      if (awardsResp && awardsResp.ok) {
        const awardsRaw = await awardsResp.text();
        const $aw = page_parser$1(awardsRaw);
...
async function fetchWithTimeout$3(url, opts = {}, timeout = DEFAULT_TIMEOUT$3) {
  const controller = new AbortController();
  const id = setTimeout(() => controller.abort(), timeout);
  try {
    const res = await fetch(url, { ...opts, signal: controller.signal });
...
async function gen_imdb(sid, env) {
  ...
  try {
    ...
    let pageResp;
    try {
      pageResp = await fetchWithTimeout$3(imdb_url, { headers: getHeaders(env) }, DEFAULT_TIMEOUT$3);
    } catch (err) {
    ...
    if (pageResp && pageResp.status === 204) {
      ...
      try {
        ...
        const mobileResp = await fetchWithTimeout$3(mobileUrl, { headers: getHeaders(env) }, DEFAULT_TIMEOUT$3);
      ...
      if (pageResp && pageResp.status === 204) {
        try {
          ...
          const sugResp = await fetchWithTimeout$3(suggestionUrl, { headers: { 'Accept': 'application/json' } }, 5000);
    ...
    try {
      const releaseResp = await fetchWithTimeout$3(`${imdb_url}releaseinfo`, { headers: getHeaders(env) }, DEFAULT_TIMEOUT$3);
      if (releaseResp && releaseResp.ok) {
        const releaseHtml = await releaseResp.text();
        const $rel = page_parser$1(releaseHtml);
...
const DEFAULT_TIMEOUT$2 = 15000;

async function fetchWithTimeout$2(url, { timeout = DEFAULT_TIMEOUT$2, ...opts } = {}) {
  const controller = new AbortController();
  const id = setTimeout(() => controller.abort(), timeout);
  try {
...
const safe$2 = (v, fallback = '') => (v === undefined || v === null ? fallback : v);
...
function formatCharacters(chars = []) {
  ...
  for (const c of chars) {
    if (!c) continue;
    const name = safe$2(c.name, '');
    const name_cn = safe$2(c.name_cn, '');
...
async function gen_bangumi(sid, env) {
  ...
  try {
    // 请求 subject 主数据
    const subjResp = await fetchWithTimeout$2(subjectUrl, { headers: BGM_API_HEADERS, timeout: 20000 });
    ...
    const [personsRes, charactersRes] = await Promise.allSettled([
      fetchWithTimeout$2(personsUrl, { headers: BGM_API_HEADERS, timeout: 15000 }),
      fetchWithTimeout$2(charactersUrl, { headers: BGM_API_HEADERS, timeout: 15000 })
    ]);
    ...
    data.name = safe$2(subject.name, '');
    data.name_cn = safe$2(subject.name_cn, '');
    data.summary = safe$2(subject.summary, '');
    ...
    data.date = safe$2(subject.date, '');
    data.year = data.date ? String(data.date).slice(0, 4) : '';
    data.platform = safe$2(subject.platform, '');
...
const DEFAULT_TIMEOUT$1 = 10000; // ms

const fetchWithTimeout$1 = async (url, opts = {}, timeout = DEFAULT_TIMEOUT$1) => {
  const controller = new AbortController();
...
const safe$1 = (v, fallback = '') => (v === undefined || v === null ? fallback : v);
...
const buildResult = (tmdb_data, media_type) => {
  ...
  data.title = media_type === 'movie' ? safe$1(tmdb_data.title) : safe$1(tmdb_data.name);
  data.original_title = media_type === 'movie' ? safe$1(tmdb_data.original_title) : safe$1(tmdb_data.original_name);
  data.overview = safe$1(tmdb_data.overview, '');
  ...
  if (media_type === 'movie') {
    data.release_date = safe$1(tmdb_data.release_date, '');
    data.year = data.release_date ? data.release_date.slice(0, 4) : '';
    data.runtime = tmdb_data.runtime ? `${tmdb_data.runtime} minutes` : '';
  } else {
    data.first_air_date = safe$1(tmdb_data.first_air_date, '');
    data.last_air_date = safe$1(tmdb_data.last_air_date, '');
  ...
  data.tmdb_rating_average = safe$1(tmdb_data.vote_average, 0);
  data.tmdb_votes = safe$1(tmdb_data.vote_count, 0);
...
async function gen_tmdb(sid, env) {
  ...
  try {
    ...
    let resp;
    try {
      resp = await fetchWithTimeout$1(url, {
        headers: {
 ...
 */
async function gen_melon(sid, env) {
...
async function gen_steam(sid, env) {
