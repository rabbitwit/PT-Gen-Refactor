...
const generateSteamFormat = (data) => {
  ...
  if (data.about_the_game) {
      ...
      const $ = page_parser$1(data.about_the_game);
      ...
      blocksHTML.forEach(blockHtml => {
          const $block = page_parser$1(blockHtml); 
...
const safe$2 = (v, fallback = '') => v ?? fallback;
...
const formatCharacters = (chars = []) => {
  ...
  for (const c of chars) {
    ...
    const nameCn = safe$2(c.name_cn);
    const actors = ensureArray(c.actors)
      .map(a => safe$2(a?.name_cn || a?.name))
      .filter(Boolean)
      .join('、') || '未知';
...
async function gen_bangumi(sid) {
  ...
  try {
    ...
    data.name_cn = safe$2(getInfoboxValue(subject.infobox, '中文名'), subject.name_cn);
...
const buildResult = (tmdb_data, media_type) => {
  ...
  const getTitleField = (fieldMovie, fieldTv) =>
    media_type === 'movie' ? safe$1(tmdb_data[fieldMovie]) : safe$1(tmdb_data[fieldTv]);
