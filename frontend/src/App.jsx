...
function App() {
  ...
  const handleSelectResult = async (link) => {
    ...
    try {
      ...
      const isDouban = link.includes("douban.com");
      ...
      if (!link) {
        ...
        if (isImdb) {
          ...
          if (imdbIdMatch && imdbIdMatch[0]) {
            ...
            apiUrl = `/api?url=${encodedLink}`;
          }
        } else if (isTmdb) {
          const tmdbIdMatch =
            link.match(/movie\/(\d+)/) || link.match(/tv\/(\d+)/);
          ...
          const doubanIdMatch = link.match(/(\d+)/);
          if (doubanIdMatch && doubanIdMatch[1]) {
            apiUrl = `/api?source=douban&sid=${doubanIdMatch[1]}`;
          } else {
            apiUrl = `/api?url=${encodedLink}`;
          }
        } else {
